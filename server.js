...
app.get('/zerads-callback', async (req, res) => {
  ...
  const amount = Number(firstQueryValue(req.query.amount) || 0);
  const clicks = Number(firstQueryValue(req.query.clicks) || 0);
  ...
  try {
    ...
    if (!doc) {
      return res.status(404).json({ ok: false, error: 'User not found by username/ptcUser.' });
    }
