...
- `ZERADS_PTC_TARGET_URL` = butona tıklanınca açılacak PTC URL şablonu (boşsa `https://zerads.com/ptc.php?ref=10799&user={user}` kullanılır)
...
### 5.2) PTC buton hedefi

Bu sürümde PTC butonu ZerAds PTC sayfasına gider:

`https://zerads.com/ptc.php?ref=10799&user={user}`

Uygulama `{user}` yerine giriş yapan kullanıcının username/ismi ile URL üretir.

Callback URL ise ZerAds panelinde API callback alanına girilir:
