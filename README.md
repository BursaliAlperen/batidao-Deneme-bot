...
- `ZERADS_PTC_TARGET_URL` = butona tıklanınca açılacak URL (boşsa callback URL kullanılır)
...
Bu sürümde PTC butonu doğrudan aşağıdaki callback URL'i açacak şekilde ayarlanmıştır:

`https://batidao-deneme-bot.onrender.com/zerads-callback?user=user&amount=amount&clicks=clicks`

İstersen bunu `ZERADS_PTC_TARGET_URL` ile değiştirebilirsin.
