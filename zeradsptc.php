...
function isTrustedIp($trustedIp, $remoteAddr, $forwardedFor)
{
    $allowed = array_filter(array_map('trim', explode(',', (string)$trustedIp)));
    if (empty($allowed)) return false;
    $candidates = [];
    if ($remoteAddr) $candidates[] = trim($remoteAddr);
    if ($forwardedFor) {
    ...
    foreach ($candidates as $ip) {
        if (in_array($ip, $allowed, true)) return true;
    }
    return false;
}
...
if (!isTrustedIp($trustedIp, $clientIp, $forwardedFor)) {
    http_response_code(403);
    echo json_encode(['ok' => false, 'error' => 'invalid_ip', 'ip' => $clientIp, 'xff' => $forwardedFor]);
    exit;
}
